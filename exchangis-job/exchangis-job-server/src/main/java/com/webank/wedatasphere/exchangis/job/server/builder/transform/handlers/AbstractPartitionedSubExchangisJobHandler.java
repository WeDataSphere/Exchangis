...
public abstract class AbstractPartitionedSubExchangisJobHandler extends AuthEnabledSubExchangisJobHandler{
     ...
     * Returns null when autoCreateTable is off (no need to check). When autoCreateTable is on,
     * any failure in checking (including data sources that do not support existsTable) fails the
     * job directly instead of silently degrading / 表是否存在；autoCreateTable 关闭时返回 null（无需判断）；
     * autoCreateTable 开启时通过 existsTable 主动判断，判断失败（含不支持该接口的数据源）直接令任务失败，不再静默降级
     */
    protected static final JobParamDefine<Boolean> TABLE_EXISTS = JobParams.define(
        ...
        // 主动判断表是否存在；接口调用或接口不支持抛出的异常直接向上抛出，令任务失败
        // proactively check table existence; failures (incl. unsupported data sources) propagate to fail the job
        try {
    ...
    protected static final JobParamDefine<List<String>> PARTITION_KEYS = JobParams.define("partitionKeys", paramSet -> {
        ...
        // 表确实不存在（autoCreateTable 开启时主动判断）→ 走自动建表降级：从 TABLE_PARTITION 取分区键，不再查询元数据
        ...
        // tableExists == null（autoCreateTable 关闭）或 tableExists == true（表存在）：正常查询分区键，异常照常抛出
        return Objects.requireNonNull(getBean(MetadataInfoService.class)).getPartitionKeys(
                Optional.ofNullable(dsOwner).orElse(getJobBuilderContext().getOriginalJob().getCreateUser()),
                Long.parseLong(dataSourceId.getValue()), database, table);
    });
